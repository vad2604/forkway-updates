...
| Файл | Ссылка |
|------|--------|
| DMG (установщик) | [Forkway-1.0.6-macos-arm64.dmg](https://vad2604.github.io/forkway-updates/releases/v1.0.6/Forkway-1.0.6-macos-arm64.dmg) |
| Контрольная сумма | [Forkway-1.0.6-macos-arm64.dmg.sha256](https://vad2604.github.io/forkway-updates/releases/v1.0.6/Forkway-1.0.6-macos-arm64.dmg.sha256) |

Проверка целостности (рекомендуется):

```bash
cd ~/Downloads
curl -LO https://vad2604.github.io/forkway-updates/releases/v1.0.6/Forkway-1.0.6-macos-arm64.dmg.sha256
curl -LO https://vad2604.github.io/forkway-updates/releases/v1.0.6/Forkway-1.0.6-macos-arm64.dmg
shasum -a 256 -c Forkway-1.0.6-macos-arm64.dmg.sha256
```
...
### Корпоративные домены (CheckPoint)

Начиная с **1.0.6**, при **Start** с подключённым CheckPoint Forkway автоматически:

- определяет корпоративный DNS (`10.x`) и сохраняет в `~/.config/forklift/settings.json`
- добавляет правила SNX для search domains CheckPoint (например `alfaintra.net`)

После обновления с более ранней версии: **Stop** → **Start** один раз при подключённом CheckPoint.

Ручная настройка (если автодетект не сработал):

```json
// ~/.config/forklift/settings.json
{
  "corporate_dns": "10.224.0.5",
  "public_dns": "1.1.1.1"
}
```
...
> Нужна версия приложения **1.0.6+** — автоконфигурация corp DNS и правил CheckPoint; **1.0.5+** — автообновление helper.
...
| `dig @127.0.0.1` → SERVFAIL для `*.alfaintra.net` | Stop → Start при подключённом CP; проверьте `ps … dnsproxy` (должен быть `-u [/…/]10.x:53`) |
| После обновления сломалось | Quit → Stop → Start снова; при необходимости переустановить helper |
